...
local QIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/c/c7/Light_Binding.png"
local WIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/7/7c/Prismatic_Barrier.png"
local EIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/3/39/Lucent_Singularity.png"
local RIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/0/06/Final_Spark.png"
...
function Lux:LoadMenu()
	self.Menu = MenuElement({type = MENU, id = "Lux", name = "Kypo's Lux", leftIcon = HeroIcon})
	...
	self.Menu.Combo:MenuElement({id	="ShieldMinHealth",name="Min Health -> %",value=20,min=0,max=100})
	...
	self.Menu.Clear:MenuElement({id = "UseE", name = "E", value = true, leftIcon = QIcon})
	self.Menu.Clear:MenuElement({id = "EHit", name = "E hits x minions", value = 4,min = 2, max = 6, step = 1, leftIcon = EIcon})
	...
	self.Menu:MenuElement({id = "Junglesteal", name = "Junglesteal", type = MENU})
	self.Menu.Junglesteal:MenuElement({id = "jungleActive", name = "Use in jungle", key = string.byte("U")})
	self.Menu.Junglesteal:MenuElement({id = "Dragon", name = "Use AutoR on: Dragon", value = true})
	self.Menu.Junglesteal:MenuElement({id = "Baron", name = "Use AutoR on: Baron", value = true})
	self.Menu.Junglesteal:MenuElement({id = "Herald", name = "Use AutoR on: Herald", value = true})
	...
	--E
	self.Menu.Drawings:MenuElement({id = "R", name = "Draw R range", type = MENU, leftIcon = EIcon})
    self.Menu.Drawings.R:MenuElement({id = "Enabled", name = "Enabled", value = true})
    self.Menu.Drawings.R:MenuElement({id = "Minimap", name = "On minimap?", value = true})       	
...
function Lux:Junglesteal()
	...
	for i, target in pairs(self:GetEnemyHeroes()) do
		if GetDistance(myHero.pos, target.pos) <= rrange then
			...
			if self.Menu.Junglesteal.jungleActive["RU"..target.charName] and self.Menu.Junglesteal.jungleActive["RU"..target.charName]:Value() and RDamage > target.health then
				self:CastSpell(HK_Q, target.pos)
			end
		end
	end
	if self.Menu.Junglesteal.jungleActive:Value() then
		local RDamage = self:GetRDMGJng()
		local RDamageBaron = self:GetRDMGBaron()
		local minionlist = {}
		if _G.SDK then
		...
		for i, minion in pairs(minionlist) do
			if self.Menu.Junglesteal.Dragon:Value() then
				if JungleTable[minion.charName] and RDamage > minion.health and self:CanCast(_R) and self:CanCast(_Q) then
			...
			if self.Menu.Junglesteal.Herald:Value() then
				if minion.charName == "SRU_RiftHerald" and RDamage > minion.health and self:CanCast(_R) and self:CanCast(_Q) then
						Control.SetCursorPos(minion.pos)
						Control.KeyDown(HK_Q)
						Control.KeyUp(HK_Q)					
						Control.CastSpell(HK_R, minion.pos)				
			...
			if self.Menu.Junglesteal.Baron:Value() then
				if minion.charName == "SRU_Baron" and RDamageBaron > minion.health and self:CanCast(_R) and self:CanCast(_Q) then
						Control.SetCursorPos(minion.pos)
						Control.KeyDown(HK_Q)
						Control.KeyUp(HK_Q)					
						Control.CastSpell(HK_R, minion.pos)
...
function Lux:HasBuff(unit, buffname)
...
function Lux:GetValidMinion(range)
    	for i = 1,Game.MinionCount() do
        local minion = Game.Minion(i)
...
function Lux:IsReady(spell)
	return Game.CanUseSpell(spell) == 0
end

function Lux:CheckMana(spellSlot)
	return myHero:GetSpellData(spellSlot).mana < myHero.mana
end

function Lux:CanCast(spellSlot)
...
function Lux:EnemyInRange(range)
	local count = 0
...
if self.Menu.Drawings.R.Enabled:Value() then Draw.Circle(myHero.pos, 3340, self.Menu.Drawings.R.Width:Value(), self.Menu.Drawings.R.Color:Value()) end
if self.Menu.Drawings.R.Minimap:Value() then Draw.CircleMinimap(h, 3340, self.Menu.Drawings.R.Width:Value(), self.Menu.Drawings.R.Color:Value()) end
...
function Lux:CastSpell(spell,pos)
...
function Lux:HpPred(unit, delay)
	if _G.GOS then
...
function Lux:KillstealR()
	...
	if self.Menu.Killsteal.RR["UseR"..target.charName]:Value() and self:CanCast(_R) then
		if self:EnemyInRange(3300) then 
			local level = myHero:GetSpellData(_R).level	
			local castpos,HitChance, pos = TPred:GetBestCastPosition(target, R.Delay , R.Width, R.Range,R.Speed, myHero.pos, R.ignorecol, R.Type )
...
function Lux:Autoshield()
	...
	if target == nil then return end
	if self.Menu.Combo.UseW:Value() and self:CanCast(_W) and myHero.health<=myHero.maxHealth * self.Menu.Combo.ShieldMinHealth:Value()/100 then
...
function Lux:Clear()
	...
	for i = 1, Game.MinionCount() do
		local minion = Game.Minion(i)
		if  self:isValidTarget(minion,1000)  then
...
function Lux:isValidTarget(obj,range)
	range = range and range or math.huge
