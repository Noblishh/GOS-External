...
function Draven:Tick()
	...
	for i = 1, Game.ParticleCount() do
        ...
        if particle.name == "Draven_Base_Q_reticle" then
            if not self.qParticles[particle.handle] then
                if myHero.pos:DistanceTo(particle.pos) < 550 then
                    self.qParticles[particle.handle] = { pos = particle.pos, tick = GetTickCount(), success = false }
                end
            end
        end
    end
    ...
    for k,v in pairs(self.qParticles) do
        ...
        elseif GetTickCount() > v.tick + 1000 then
...
if self.Menu.Drawings.E.Enabled:Value() then Draw.Circle(myHero.pos, 1100, self.Menu.Drawings.E.Width:Value(), self.Menu.Drawings.E.Color:Value()) end			
	...
	for k,v in pairs(self.qParticles) do
        if not v.success then
            Draw.Circle(v.pos, 225, 1, Draw.Color(200, 255, 255, 255))
        end
    end
...
function Draven:QGrab()
	if self.Menu.Combo.CatchQ:Value() then
	for i = 1, Game.ParticleCount() do
        local particle = Game.Particle(i)
	for k,v in pairs(self.qParticles) do
        if not v.success then
		if myHero.pos:DistanceTo(particle.pos) < 550 then
			MoveToParticle(particle.pos)
			-- Control.SetCursorPos(particle.pos)
			-- DelayAction(RightClick, particle.pos, 1.0) end end
		    end	
		end
	end
	end
...
function Draven:QCast(spell,pos)
	local customcast = self.Menu.CustomSpellCast:Value()
	if not customcast then
		Control.CastSpell(spell, pos)
		return
